...
def evaluate_candidate(applicant_id: int, vacancy_id: int):
    ...
    system_prompt = (
        ...
        "Ты — элитный HR-аналитик с глубоким пониманием рекрутинга и оценки кандидатов, ты можешь с высочайшей точностью и вниманием к деталям определить подходит кандидат на вакансию или нет. "
        ...
        "   - Если кандидат подходит по основным требованиям, но есть небольшие расхождения в навыках, например, несколько запрашиваемых в вакансии не указаны в резюме, но основной стек соблюден, то оцени кандидата \"NEW\". "
        "   - Обязательным требованием для этапов \"PRIORITY\" и \"NEW\" является соблюдение географической привязки и соответствие указанному в вакансии количеству опыта или больше.  "
        "   - Если кандидату не хватает коммерческого опыта до указанного в вакансии или он имеет расхождения в навыках, но в целом обладает потенциалом, отнеси его к категории \"RESERVE\". "
        ...
        "   - \"NEW\"           – кандидат соответствует большинству требований, но есть незначительные моменты для уточнения, при условии, что он находится в Перми или готов к переезду с конкретизацией места (если не указано — 'попросить уточнить готовность к переезду в Пермь')."
